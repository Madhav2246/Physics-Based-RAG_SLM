...
def _load_corpus_chunks(corpus_dir: Path) -> List[str]:
    """Load all .txt files from corpus dir, split into paragraphs."""
    chunks = []
    for txt_file in sorted(corpus_dir.glob("*.txt")):
        text = txt_file.read_text(encoding="utf-8", errors="replace")
        paragraphs = [p.strip() for p in re.split(r'\n{2,}', text) if len(p.split()) > 40]
...
def generate_dataset(corpus_dir: Path, reset: bool = False):
    ...
    for i, difficulty in enumerate(todo):
        ...
        while attempts < 8:
            chunk = shuffled_chunks[chunk_idx % len(shuffled_chunks)]
            chunk_idx += 1
            attempts  += 1
            ...
            entry = {
                ...
                "equation":     equation,
                "symbols":      symbols,
                "source_chunk": chunk[:300] + "…" if len(chunk) > 300 else chunk,
