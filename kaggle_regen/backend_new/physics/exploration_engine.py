...
class ExplorationEngine:
    ...
    def solve_sweep(self, lhs_expr, rhs_expr, query: str,
        ...
        """
        # Prefer target_var from the parsed sweep request (derived from "A vs B"
        # syntax) over detect_target(), which pattern-matches the full query and
        # can collide with sweep_var when both variables appear in the text.
        target = sweep_req.target_var or self.detect_target(query)
        if target is None:
        ...
        # solve_for's numeric step fails when sweep_var is not in the tracker
        # (it's intentionally free — the sweep axis). Clear that error here;
        # run_sweep handles substitution and catches truly-unresolved symbols.
        if result.get('error') and result.get('symbolic_expr') is not None:
    ...
    @staticmethod
    def _fmt_param(param: str, v: float) -> str:
        """Compact engineering label for a family-of-curves parameter value."""
        units = {"Id": "A", "Ids": "A", "Iref": "A", "Idsat": "A",
                 "Vov": "V", "Vgs": "V", "Vds": "V", "Vth": "V", "Vsb": "V"}
        u = units.get(param, "")
        if u == "A":
            return f"{v*1e6:.3g} uA" if abs(v) < 1e-3 else f"{v*1e3:.3g} mA"
        if u == "V":
            return f"{v:.3g} V"
        return f"{v:.4g}"

    def solve_sweep_multi(self, lhs_expr, rhs_expr, query: str, sweep_req,
                          param: str, values: list, node_defaults=None) -> dict:
        """
        Family-of-curves SWEEP. Derives the symbolic solution once (via
        solve_sweep), then vectorizes it over the sweep range for EACH value of
        `param` — a quantity held FIXED per curve but varied across curves
        (e.g. "... for Id = 0.5mA, 1mA, 2mA"). Returns the solve_sweep dict plus:
          'multi_results' : list[SweepResult]   (one per param value)
          'multi_labels'  : list[str]           (legend labels)
          'multi_param'   : str
        multi_results is None if the base solve fails or `param` collides with
        the sweep/target axis (caller then falls back to a single-curve sweep).
        """
        base = self.solve_sweep(lhs_expr, rhs_expr, query, sweep_req,
                                node_defaults=node_defaults)
        if not base.get('success') or base.get('symbolic_expr') is None:
            base['multi_results'] = None
            return base

        target = base.get('target_var') or sweep_req.target_var
        if param == target or param == sweep_req.sweep_var:
            base['multi_results'] = None
            base['error'] = (f"Family parameter '{param}' collides with the "
                             f"sweep/target axis.")
            return base

        import sympy as _sp
        sym_expr = base['symbolic_expr']
        # Mirror solve_sweep's Vov-axis fix so curves use overdrive correctly.
        if sweep_req.sweep_var == 'Vov':
            vgs, vth, vov = _sp.Symbol('Vgs'), _sp.Symbol('Vth'), _sp.Symbol('Vov')
            if vgs in sym_expr.free_symbols:
                sym_expr = sym_expr.subs(vgs, vov + vth)

        results, labels = [], []
        for v in values:
            tracker = self.extract_user_values(query, node_defaults=node_defaults)
            tracker.add_user(param, float(v), "", "family-of-curves parameter")
            sr = self._sweep_engine.run_sweep(sweep_req, sym_expr, tracker, target)
            if sr.error:
                base['multi_results'] = None
                base['error'] = f"Family curve {param}={v}: {sr.error}"
                return base
            results.append(sr)
            labels.append(f"{param} = {self._fmt_param(param, v)}")

        base['multi_results'] = results
        base['multi_labels'] = labels
        base['multi_param'] = param
        base['response'] = (
            f"Equation: {base['symbolic']}\n"
            f"[SWEEP MODE — family of curves] {target} vs {sweep_req.sweep_var} "
            f"for {len(values)} values of {param}.\n"
            f"  Algebra (deterministic): SymPy solve() — no SLM call\n"
            f"  Node profile: {sweep_req.node_name}"
        )
        return base
