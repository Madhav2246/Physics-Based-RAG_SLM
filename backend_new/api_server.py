...
# HF cache lives next to backend_new (project root). Computed from this file's
# location so it survives moving the repo between drives/machines — do NOT
# hardcode an absolute path here (the old "d:/S6/..." path broke on this box).
_HF_CACHE = PROJECT_ROOT.parent / "hf_cache"
os.environ["HF_HOME"] = str(_HF_CACHE)
# Offline only if the cache is actually present; otherwise allow downloads so a
# fresh checkout can populate it instead of crashing.
if (_HF_CACHE / "hub").exists():
...
class QueryBody(BaseModel):
    question: str
    # UI mode override: None/"AUTO" → keyword auto-detection; otherwise force
    # one of LOOKUP | EXPLORE | SWEEP in the pipeline.
...
async def rag_query(body: QueryBody):
    ...
    loop = asyncio.get_event_loop()
    # body.mode (None/AUTO → auto-detect) is passed as answer(query, force_mode)
    result = await loop.run_in_executor(
        _executor, _pipeline.answer, body.question, body.mode)
