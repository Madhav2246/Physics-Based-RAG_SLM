...
class RAGPipeline:
    ...
    def _find_corpus_equation(self, evidence: list, prefer_vars: set | None = None):
        """
        Scan retrieved chunks for a parseable, physics-relevant equation.
        Quality filters reject OCR garbage / PDF artifacts / letter-soup.
        Returns (lhs_expr, rhs_expr, equation_str) or (None, None, None).

        prefer_vars (SWEEP/EXPLORE): the variables the user wants to plot/solve.
        When given, the extractor scans EVERY candidate in every chunk and
        returns an equation whose free symbols cover all prefer_vars — so
        "plot ro vs Id" grounds on an equation containing ro AND Id instead of
        the first MOSFET equation in the chunk. If no candidate matches, the
        first otherwise-acceptable equation is returned as a fallback. With
        prefer_vars=None the original LOOKUP behaviour is unchanged.
        """
        import sympy as _sp

        def _acceptable(lhs, rhs):
            """Return the symbol-name set if the parsed eq is a real equation,
            else None. Rejects 2=3, single-symbol, and letter-soup."""
            if not isinstance(lhs, _sp.Basic) or not isinstance(rhs, _sp.Basic):
                return None                                # real exprs
            if lhs.is_number and rhs.is_number:
                return None                                # not 2=3
            syms = lhs.free_symbols | rhs.free_symbols
            if len(syms) < 2:
                return None                                # >=2 symbols
            names = {s.name for s in syms}
            # Letter-soup guard: a genuine equation has a multi-character symbol
            # OR almost every symbol is recognised. Rejects artefacts like
            # "G*S*v - T*V = -0.06*V*l*n*o*s*t*x" extracted from prose.
            multi = any(len(n) > 1 for n in names)
            known = sum(1 for n in names if n in self.validator.symbols)
            if not multi and known < len(names) - 1:
                return None
            return names

        # ── Variable-aware path (SWEEP / EXPLORE) ────────────────────────────
        if prefer_vars:
            fallback = None
            for chunk in evidence:
                for cand in self.validator._candidate_equations(chunk):
                    c_lhs, c_rhs, c_msg = self.validator.validate(cand)
                    if c_lhs is None or "[OK]" not in c_msg:
                        continue
                    names = _acceptable(c_lhs, c_rhs)
                    if names is None:
                        continue
                    if prefer_vars <= names:
                        return c_lhs, c_rhs, f"{c_lhs} = {c_rhs}"
                    if fallback is None:
                        fallback = (c_lhs, c_rhs)
            if fallback is not None:
                return fallback[0], fallback[1], f"{fallback[0]} = {fallback[1]}"
            return None, None, None

        # ── General path (LOOKUP) ────────────────────────────────────────────
        # Scan candidate equations in each chunk and apply the SAME letter-soup
        # guard (_acceptable) as the prefer_vars path. Previously this validated
        # the whole chunk as one expression with no letter-soup filter, so prose
        # like "current density" parsed to garbage (I = N*d*e*i**2*j*r*t) and was
        # returned as the corpus equation — driving Stage-1/4 coverage to 0.
        # Prefer a candidate whose symbols are mostly RECOGNISED physics symbols;
        # fall back to the first merely-acceptable one.
        fallback = None
        for chunk in evidence:
            for cand in self.validator._candidate_equations(chunk):
                c_lhs, c_rhs, c_msg = self.validator.validate(cand)
                ...
                names = _acceptable(c_lhs, c_rhs)
                if names is None:
                    continue                               # letter-soup rejected
                known = sum(1 for n in names if n in self.validator.symbols)
                if known >= 2:                             # strong: real physics eq
                    return c_lhs, c_rhs, f"{c_lhs} = {c_rhs}"
                if fallback is None:
                    fallback = (c_lhs, c_rhs)
        if fallback is not None:
    ...
    @staticmethod
    def _eq_covers(lhs, rhs, vars_: set | None) -> bool:
        """True if the equation's free symbols contain all of vars_."""
        if lhs is None or not vars_:
            return False
        try:
            names = {s.name for s in (lhs.free_symbols | rhs.free_symbols)}
        except Exception:
            return False
        return vars_ <= names
    ...
    def answer(self, query: str, force_mode: str | None = None) -> dict:
        import re as _re
        ...
        # -- 2  Mode: SWEEP vs EXPLORE vs LOOKUP ─────────────────────────────
        # SWEEP:   plot a variable over a range (most specific).
        # EXPLORE: solve/design for a single point.
        # LOOKUP:  explain an equation (default, safe path).
        # A caller (UI mode button) may FORCE the mode via force_mode; an
        # empty/"AUTO"/invalid value falls back to keyword auto-detection.
        forced = (force_mode or "").strip().upper()
        mode = forced if forced in ("LOOKUP", "EXPLORE", "SWEEP") else detect_mode(query)
        explore_result = None
        sweep_plot_path = None

        # Variables the user wants to plot/solve, so design queries ground on an
        # equation that actually CONTAINS them ("plot ro vs Id" → eq with ro,Id),
        # not the first MOSFET equation that happens to appear in a chunk.
        prefer_vars = None
        if mode == "SWEEP":
            from physics.sweep_engine import parse_sweep_request as _psr
            from physics.sweep_engine import parse_multi_curve_param as _pmc
            _sr = _psr(query)
            if _sr is not None:
                prefer_vars = {v for v in (_sr.target_var, _sr.sweep_var) if v}
                _m = _pmc(query)
                if _m is not None:
                    prefer_vars.add(_m[0])   # family param must be in the eq too
        elif mode == "EXPLORE":
            try:
                _tgt = self.exploration_engine.detect_target(query)
                if _tgt:
                    prefer_vars = {_tgt}
            except Exception:
                prefer_vars = None

        # -- 3  Extract equation from corpus chunks (variable-aware in design) -
        corpus_lhs_expr, corpus_rhs_expr, corpus_equation = \
            self._find_corpus_equation(evidence, prefer_vars=prefer_vars)
        ...
        # equation. Retry on a cleaned query when nothing was found OR when the
        # found equation does not contain the requested variables.
        # LOOKUP queries never trigger this — their retrieval path is unchanged.
        need_retry = mode in ("SWEEP", "EXPLORE") and (
            corpus_lhs_expr is None
            or (prefer_vars and not self._eq_covers(
                    corpus_lhs_expr, corpus_rhs_expr, prefer_vars)))
        if need_retry:
            cleaned = self._clean_design_query(query)
            if cleaned and cleaned.lower() != query.lower():
                retry_ev = self.retriever.retrieve(cleaned, top_k=cfg.TOP_K)
                r_lhs, r_rhs, r_eq = self._find_corpus_equation(retry_ev, prefer_vars=prefer_vars)
                # Take the retry equation only if it matches the wanted vars, or
                # if we had no usable equation at all before.
                if r_lhs is not None and (
                        self._eq_covers(r_lhs, r_rhs, prefer_vars)
                        or corpus_lhs_expr is None):
        ...
        if mode == "SWEEP" and corpus_lhs_expr is not None:
            from physics.sweep_engine import parse_sweep_request, parse_multi_curve_param
            sweep_req = parse_sweep_request(query)
            multi = parse_multi_curve_param(query)   # "for Id=0.5mA, 1mA, 2mA"
            if sweep_req is not None:
                sweep_req.node_name = node_name or "100nm_CMOS"
                try:
                    if multi is not None and multi[0] not in (
                            sweep_req.target_var, sweep_req.sweep_var):
                        # Family of curves: one line per value of multi[0].
                        m_param, m_values = multi
                        explore_result = self.exploration_engine.solve_sweep_multi(
                            corpus_lhs_expr, corpus_rhs_expr, query, sweep_req,
                            m_param, m_values, node_defaults=node_defaults,
                        )
                        mres = explore_result.get("multi_results")
                        if explore_result.get("error") or not mres:
                            mode = "EXPLORE"
                            explore_result = None
                        else:
                            out = (_PROJECT_ROOT / "data" / "evaluation" /
                                   f"sweep_{sweep_req.target_var or 'result'}_vs_"
                                   f"{sweep_req.sweep_var}_{m_param}_family.png")
                            tv = explore_result.get('target_var') or sweep_req.target_var
                            sweep_plot_path = self.exploration_engine._sweep_engine.plot_multi(
                                mres, explore_result.get("multi_labels"), str(out),
                                title=f"{tv} vs {sweep_req.sweep_var} — "
                                      f"{m_param} family  ({sweep_req.node_name})",
                            )
