...
class EquationValidator:
    ...
    def normalize_equation(self, eq: str) -> str:
        ...
        # U+00B5 MICRO SIGN and U+03BC GREEK LETTER MU both appear in model output
        eq = eq.replace("µ", "mu").replace("μ_n", "mu").replace("mu_n", "mu").replace("μ", "mu")
        eq = eq.replace("ω", "omega").replace("Ω", "Omega")
        ...
        # -- SPICE/circuit-sim lowercase voltage notation (vXX → Vxx) ---------
        # Model output often uses SPICE-style vDS, vGS, vBS, vGD, vGB notation
        eq = re.sub(r'\bvDS\b', "Vds", eq)
        eq = re.sub(r'\bvGS\b', "Vgs", eq)
        eq = re.sub(r'\bvBS\b', "Vbs", eq)
        eq = re.sub(r'\bvBD\b', "Vbd", eq)
        eq = re.sub(r'\bvGD\b', "Vgd", eq)
        eq = re.sub(r'\bvGB\b', "Vgb", eq)
        eq = re.sub(r'\bvIN\b|\bvIN1\b|\bvIN2\b', "Vin", eq)
        eq = re.sub(r'\bvOUT\b|\bvout\b', "Vout", eq)
        eq = re.sub(r'\bvSB\b', "Vsb", eq)

        # -- Fix A: Domain alias normalisation --------------------------------
        # Threshold voltage variants (must come before generic Vth pattern)
        eq = re.sub(r'\bVTn\b|\bVT\b',   "Vth",   eq)   # VT, VTn → Vth
        eq = re.sub(r'\bVTp\b',           "Vth",   eq)   # VTp → Vth
        # Oxide thickness variants
        eq = re.sub(r'\bTox\b|\bTOX\b',   "tox",   eq)   # Tox, TOX → tox
        # Silicon/oxide permittivity variants
        eq = re.sub(r'\bepsSi\b|\beps_Si\b|\bepsilon_Si\b|\bepsilon_s\b', "eps_s",  eq)
        eq = re.sub(r'\bepsOx\b|\beps_ox\b|\bepsilon_ox\b',               "eps_ox", eq)
        # Small-signal AC notation (differential-pair / op-amp)
        eq = re.sub(r'\bv_?id\b|\bvid\b', "Vid",  eq)   # differential input
        eq = re.sub(r'\bv_?od\b|\bvod\b', "Vod",  eq)   # differential output
        eq = re.sub(r'\bv_?ic\b|\bvic\b', "Vcm",  eq)   # common-mode input
        eq = re.sub(r'\bv_?oc\b|\bvoc\b', "Vcm",  eq)   # common-mode output
        # Differential gain symbol
        eq = re.sub(r'\bA_?vd\b|\bAvd\b', "Av",   eq)   # Avd → Av
        eq = re.sub(r'\bA_?vc\b|\bAvc\b', "Av",   eq)   # Avc → Av
        # Transconductance variants
        eq = re.sub(r'\bg_?m\b',          "gm",   eq)   # g_m → gm
        eq = re.sub(r'\bg_?ds\b',         "gds",  eq)   # g_ds → gds
        eq = re.sub(r'\bg_?mb\b',         "gmb",  eq)   # g_mb → gmb
        # Output resistance variants
        eq = re.sub(r'\br_?o\b|\bRo\b',   "ro",   eq)   # r_o, Ro → ro
        # Supply / bias voltages
        eq = re.sub(r'\bV_?DD\b|\bVDD\b', "Vdd",  eq)
        eq = re.sub(r'\bV_?CC\b|\bVCC\b', "Vcc",  eq)
        eq = re.sub(r'\bV_?SS\b',         "Vdd",  eq)   # treat VSS = supply
        # Gate-source / drain-source capacitances
        eq = re.sub(r'\bC_?gs\b',         "Cgs",  eq)
        eq = re.sub(r'\bC_?gd\b',         "Cgd",  eq)
        eq = re.sub(r'\bC_?db\b',         "Cdb",  eq)
        # Flatband voltage
        eq = re.sub(r'\bV_?fb\b|\bV_?FB\b', "Vfb", eq)
        # Number of fingers / segments
        eq = re.sub(r'\bN_?f\b|\bNfing\b',  "Nf",  eq)
        ...
        # -- Implicit physics compound symbols → explicit products -----------
        # These appear without * in handwritten/model physics notation
        eq = re.sub(r'\bqV\b',  'q*V',    eq)   # charge * voltage
        eq = re.sub(r'\bnkT\b', 'n*k*T',  eq)   # ideality * Boltzmann * temp
        eq = re.sub(r'\bkT\b',  'k*T',    eq)   # thermal energy
        eq = re.sub(r'\bqNA\b', 'q*NA',   eq)   # charge * acceptor doping
        eq = re.sub(r'\bqND\b', 'q*ND',   eq)   # charge * donor doping
        eq = re.sub(r'\bqni\b', 'q*ni',   eq)   # charge * intrinsic density
