...
* **What it is:** We ran our 0.5B system and the 70B system on 100 questions and scored them using the SymPy physics validator.
* **Why it matters:** This answers the ultimate question: *Does it work?* It proved that our tiny model beats a massive model on hard questions (+37%).

### Stage 2: The Sanity Check (Generation Quality)
* **What it is:** We measured Faithfulness, BERTScore, and ROUGE.
* **Why it matters:** Stage 1 only looked at math. Stage 2 proves the model is actually speaking English properly and not just hallucinating random words next to the math. It proved our system is 4.6× more faithful to the documents than the 70B model.
...
* **Why it matters:** It proves *why* the system works. It proved that the corpus equation prepend was the most important part of the architecture (+0.781 score boost).
...
* **Our result:** Our effect size for SYS vs RAW was **0.621**. That is a *massive* effect size, proving the architecture radically changes the model's behavior.
...
> **A:** "Because large models win on easy, rote-memorization questions due to their massive parameter count. But on hard questions, parametric memory fails. Our system explicitly trades memorization for structural reasoning, which is why we achieve a +37% advantage specifically on the hardest engineering questions."
